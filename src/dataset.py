...
def prepare_datasets(args, transform=None):
    # train dataset
...
def prepare_dataset(configs, split, transform=None):
    ...
    if transform is not None:
        ...
        image_transform = transforms.Compose([
            ...
            transforms.RandomHorizontalFlip()])
    # train dataset
    dataset = TextImgDataset(split=split, transform=image_transform, configs=configs)
    return dataset


def get_fix_data(train_dl, test_dl, text_encoder, args):
    fixed_image_train, _, _, fixed_sent_train, fixed_word_train, fixed_key_train = get_one_batch_data(train_dl,
                                                                                                      text_encoder,
                                                                                                      args)
    fixed_image_test, _, _, fixed_sent_test, fixed_word_test, fixed_key_test = get_one_batch_data(test_dl, text_encoder,
                                                                                                  args)
...
class TextImgDataset(data.Dataset):
    def __init__(self, split, transform=None, configs=None):
    ...
    def __getitem__(self, index):
        ...
        caps = get_caption(text_name)
        return imgs, caps, key
